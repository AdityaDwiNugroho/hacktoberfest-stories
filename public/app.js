...
async function loadStories() {
  try {
    ...
    allStories = data.stories.sort((a, b) => b.id - a.id);
...
function displayStories(stories) {
  ...
  if (stories.length === 0) {
    container.innerHTML =
      '<p class="no-results">No stories found. Be the first to share!</p>';
    return;
  }
  ...
  container.innerHTML = paginatedStories
    .map((story) => {
      const wordCount = story.story.split(" ").length;
      const isLong = wordCount > 100;
      const preview = isLong
        ? story.story.split(" ").slice(0, 100).join(" ") + "..."
        : story.story;

      return `
        <div class="story-card" data-story-id="${story.id}">
            <div class="story-header" onclick="window.open('${
              story.githubUrl
            ...
            <p class="story-text" id="story-${story.id}">
                ${
                  isLong
                    ? `<span class="preview">${preview}</span><span class="full-text" style="display: none;">${story.story}</span>`
                    : story.story
                }
            </p>
            ${
              isLong
                ? `<button class="read-more" onclick="toggleStory(${story.id}); event.stopPropagation();">Read More</button>`
                : ""
            }
...
function updatePagination(stories) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    if (
      ...
      paginationHTML += `<button onclick="changePage(${i})" class="page-btn ${
